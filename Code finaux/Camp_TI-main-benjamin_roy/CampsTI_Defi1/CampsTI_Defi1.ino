...
void setup() {
  ...
  // Initialise l'entrée (INPUT) pour lire la valeur des potentiomètre
...
void loop() {
  ...
  int pot2Value = analogRead(POT_PINCE_PIN);
  ...
  Serial.println("Pot1: " + String(pot1Value) + ", A: " + String(pot1_cmd) + " Pot2: " + String(pot2Value) + ", A: " + String(pot2_cmd) + " Pot3: " + String(pot3Value) + ", A: " + String(pot3_cmd));

  // Envoie de la commande moteur
  BaseServo.write(pot1_cmd);
  PinceServo.write(pot2_cmd);
